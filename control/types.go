...
// Error represents a non-2xx API response. It implements the error
// interface. Use [errors.As] to extract it from a returned error:
//
...
//	}
//
// Note that 5xx errors may have been retried several times before being
// returned — see [NewClient] for retry configuration.
type Error struct {
...
// Error returns a human-readable string in the form "message (code: N)".
// If Code is zero, only the message is returned.
...
// AppPost is the request body for [Client.CreateApp].
// Only Name is required; all pointer fields are optional and omitted
// from the request when nil.
type AppPost struct {
...
// AppPatch is the request body for [Client.UpdateApp]. This is a partial
// update — only non-zero fields are sent.
...
// AppResponse is the response for app operations. Timestamps (Created,
// Modified) are Unix milliseconds.
...
// KeyPost is the request body for [Client.CreateKey].
// Capability maps channel/resource names to permitted operations
...
// KeyPatch is the request body for [Client.UpdateKey]. This is a partial
// update — only non-zero fields are sent.
...
// NamespacePost is the request body for [Client.CreateNamespace].
...
// Queue is the request body for [Client.CreateQueue]. Region must match
// an available Ably region (e.g. "us-east-1-a").
type Queue struct {
	Name      string `json:"name"`
	TTL       int    `json:"ttl"`
...
// RuleResponse is the response returned by all rule operations. The
// Target field is deserialized as an untyped map (map[string]any in
// practice); to work with target-specific fields, marshal it back to
// JSON and decode into the appropriate target struct.
...
// [Client.GetAccountStats]. Entries is an untyped map whose keys and
// structure depend on the stats schema version.
...
// Me is the response for [Client.Me]. Fields may be nil depending on
...
// StatsParams holds query parameters for stats endpoints. Pass nil to
// [Client.GetAppStats] or [Client.GetAccountStats] to use API defaults.
