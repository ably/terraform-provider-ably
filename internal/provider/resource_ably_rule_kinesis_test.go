		...
		Steps: []resource.TestStep{
			// Create and Read testing of ably_app.app0
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("ably_rule_kinesis.rule0", "target.region", "us-west-1"),
					resource.TestCheckResourceAttr("ably_rule_kinesis.rule0", "target.stream_name", "rule0-testing"),
					resource.TestCheckResourceAttr("ably_rule_kinesis.rule0", "target.partition_key", "message name: #{message.name},	clientId: #{message.clientId}"),
					resource.TestCheckResourceAttr("ably_rule_kinesis.rule0", "target.enveloped", "false"),
					resource.TestCheckResourceAttr("ably_rule_kinesis.rule0", "target.format", "json"),
			...
			// ImportState testing of ably_rule_kinesis.rule0
			{
				ResourceName:      "ably_rule_kinesis.rule0",
				ImportState:       true,
				ImportStateVerify: true,
				ImportStateIdFunc: func(s *terraform.State) (string, error) {
					rs, ok := s.RootModule().Resources["ably_rule_kinesis.rule0"]
					if !ok {
						return "", fmt.Errorf("resource not found")
					}
					appID := rs.Primary.Attributes["app_id"]
					if appID == "" {
						return "", fmt.Errorf("app_id attribute not set for ably_rule_kinesis.rule0")
					}
					if rs.Primary.ID == "" {
						return "", fmt.Errorf("ID not set for ably_rule_kinesis.rule0")
					}
					return fmt.Sprintf("%s,%s", appID, rs.Primary.ID), nil
				},
			...
			// Update and Read testing of ably_app.app0
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("ably_rule_kinesis.rule0", "source.channel_filter", "^my-channel.*"),
					resource.TestCheckResourceAttr("ably_rule_kinesis.rule0", "source.type", "channel.message"),
					resource.TestCheckResourceAttr("ably_rule_kinesis.rule0", "target.region", "us-east-1"),
					resource.TestCheckResourceAttr("ably_rule_kinesis.rule0", "target.stream_name", "rule0-testing"),
					resource.TestCheckResourceAttr("ably_rule_kinesis.rule0", "target.partition_key", "message name: #{message.name}, clientId: #{message.clientId}"),
					resource.TestCheckResourceAttr("ably_rule_kinesis.rule0", "target.enveloped", "true"),
					resource.TestCheckResourceAttr("ably_rule_kinesis.rule0", "target.format", "json"),
